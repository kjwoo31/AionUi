...
/**
 * Migration v14 -> v15: Add 'slack' to conversations source CHECK constraint
 */
const migration_v15: IMigration = {
  version: 15,
  name: 'Add slack to conversations source constraint',
  up: (db) => {
    db.exec(`
      CREATE TABLE IF NOT EXISTS conversations_new (
        id TEXT PRIMARY KEY,
        ...
        model TEXT,
        status TEXT CHECK(status IN ('pending', 'running', 'finished')),
        source TEXT CHECK(source IS NULL OR source IN ('aionui', 'telegram', 'lark', 'dingtalk', 'slack')),
        channel_chat_id TEXT,
        created_at INTEGER NOT NULL,
        updated_at INTEGER NOT NULL,
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
    ...
    console.log('[Migration v15] Added slack to conversations source constraint');
  },
  down: (db) => {
    db.exec(`
      UPDATE conversations SET source = NULL WHERE source = 'slack';
    `);
    ...
    console.log('[Migration v15] Rolled back: Removed slack from conversations source constraint');
  },
};
